...
if ($method == 'fetch_account') {
	$name = $_POST['name'];
	$c = 0;
	$query = "SELECT * FROM accounts WHERE role = '".$_SESSION['role']."' ";
	...
	if ($stmt->rowCount() > 0) {
		foreach($stmt->fetchAll() as $j){
			...
			echo '<tr style="cursor:pointer;" class="modal-trigger" data-toggle="modal" data-target="#upaccA" onclick="account_details(&quot;'.$j['id'].'~!~'.$j['fname'].'~!~'.$j['role'].'~!~'.$j['password'].'~!~'.$j['username'].'&quot;)">';
			echo '<td style="display:none;" >'.$j['id'].'</td>';
				echo '<td>'.$c.'</td>';
				echo '<td>'.$j['fname'].'</td>';
				echo '<td style="display:none;">'.$j['role'].'</td>';
