...
if ($method == 'admin_update') {
	...
	if ($stmt->rowCount() < 1) {
		...
		$query .= " SET remarks = '$remarks', auth_year = '$auth_year', date_authorized = '$date_authorized', expire_date = '$expire_date', dept = '$dept', i_status = 'Pending' WHERE id = '$id'";
		$stmt = $conn->prepare($query);
		if (!$stmt->execute()) {
			$error++;
		}
		...
		$query .= " SET r_of_cancellation = '$r_of_cancellation', d_of_cancellation = '$d_of_cancellation', r_status = 'Pending', dept = '$dept' WHERE auth_no = '$auth_no'";
		$stmt = $conn->prepare($query);
		if (!$stmt->execute()) {
			$error++;
		}
